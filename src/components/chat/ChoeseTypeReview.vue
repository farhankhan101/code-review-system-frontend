<template>
    <div class="sm:w-1/2 mt-10 sm:mt-0 p-4 sm:p-0 flex flex-col sm:items-stretch items-center">
        ...
        >
          <span
            :class="[
              ...
              selectedOptions.includes(option.value) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
            ]"
          ></span>
          <span
            :class="[
...
export default defineComponent({
  ...
  setup() {
    ...
    const saveToLocalStorage = () => {
      ...
      alert("Selections saved to local storage!");
    };

    return { options, selectedOptions, toggleOption, saveToLocalStorage };
  },
});
</script>
